...
function Fretboard(props) {
  ...
  function outputStrings() {
    ...
    let stringNotes = [
      [(4 + 12 - props.detune) % 12],
      [(11 + 12 - props.detune) % 12],
      [(7 + 12 - props.detune) % 12],
      [(2 + 12 - props.detune) % 12],
      [(9 + 12 - props.detune) % 12],
      [(4 + 12 - props.detune) % 12],
    ];
    ...
    <div className="fretboard-window" id={props.fretboardType}>
      ...
      <Draggable bounds={{ left: 0, right: 510 }} axis="x">
        <div
          id="fretbox"
          className={
            props.numberOfStrings[0] === 1
              ? "draggable-7"
              : props.numberOfStrings[0] === 2
              ? "draggable-8"
              : "draggable"
          }
          style={{ display: props.showHideBox ? "block" : "none" }}
        ></div>
      </Draggable>
